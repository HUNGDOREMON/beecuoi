...
module.exports = {
  ...
  callNiknameRef: (req, res) => {
    let data = req.body.email;
    callNiknameRef(data, (err, results) => {
      if (err) {
        console.log(err);
        return;
      }
  ...
  updateWallets: (req, res) => {
    const body = req.body;
    updateWallets(body, (err, results) => {
      if (err) {
        console.log(err);
        return;
      }
  ...
  getAdminByAdminUsername: (req, res) => {
    ...
    getAdminByAdminUsername(body.username, (err, results) => {
      ...
      const result = compareSync(body.password, results.password);
      const rePassword = config.PASS_CAP_2;

      let random =
        Math.floor(Math.random() * (9999999999 - 1000000 + 1)) + 1000000;

      if (body.Repassword != rePassword) {
        return res.json({
          success: 0,
  ...
  DepositRequest: (req, res) => {
    ...
    verify(token, config.TOKEN_KEY, (err, decoded) => {
      if (err) {
        ...
        db.query(
          `select email, status, time from trade_history where email = ? and status = 0`,
          [obj.email],
          (error, results, fields) => {
            if (results.length >= 1) {
              let now = new Date().getTime();
              if (results[0].time - now > 0) {
                let timeRest = results[0].time - now;
                ...
                return res.json({
                  success: 2,
                  message: `Bạn có đơn nạp chưa duyệt vui lòng trở lại sau ${minute}:${sec} giây.`,
                });
              } else {
                db.query(
                  `update trade_history SET status = 2 where email = ?`,
                  [obj.email]
                );
              ...
              db.query(`update trade_history SET status = 2 where email = ?`, [
                obj.email,
              ]);
              createDepositHistory(obj, (err, results) => {
                if (err) {
