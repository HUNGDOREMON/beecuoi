...
module.exports = {
  ...
  getAllUser: (callback) => {
    db.query(
      `SELECT users.*, account.balance FROM users JOIN account ON users.email = account.email WHERE account.type = 1 AND users.deleted_at IS NULL ORDER BY id DESC`,
      [],
      (error, results, fields) => {
        if (error) {
          return callback(error);
  ...
  _getAllUser: (callback) => {
    db.query(
      `SELECT email,nick_name,address_USDT,privateKey_USDT FROM users WHERE active = 1 AND deleted_at IS NULL ORDER BY id DESC`,
      [],
      (error, results, fields) => {
        if (error) {
          return callback(error);
  ...
  callNiknameRef: (data, callback) => {
    db.query(
      `SELECT users.email FROM users WHERE ref_code = ?`,
      [data],
      (error, results, fields) => {
        if (error) {
          return callback(error);
  ...
  updateWallets: (data, callback) => {
    let qr = `update users set address_USDT = ? WHERE id = ?`;
      db.query(
        qr,
        [
          data.wallet,
  ...
  createDepositHistory: (data, callback) => {
    let timeEnd = +new Date() + 1000 * (60 * 3 + 0) + 500;
    db.query(
      `insert into trade_history (email, from_u, to_u, type_key, type, currency, amount, note, status, created_at, time)
                            values(?,?,?,?,?,?,?,?,?,now(),?)`,
      [
        data.email,
        data.nick,
